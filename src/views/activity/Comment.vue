...
export default {
  ...
  methods: {
    ...
    fetchData(page=1) {
      ...
      topic.getRemarkList({"page": this.page, "limit": this.limit, "type": 4})
        .then(response => {
    ...
    // 根据id删除评论
    removeById(id) {
      topic.removeRemark({"id": id, "type": 4}).then(response => {
        console.log("删除成功")
    ...
    searchByName(page=1) {
      ...
      topic.searchRemarkByName({"page": this.page, "limit": this.limit, "type": 4, "key": this.keyword})
        .then(response => {
          this.list = response.data.data?.result
          this.total = response.data.data?.total
        })
